...
import { actions as actionsUser } from './../../redux/user';
import { actions as actionsSneakers } from './../../redux/sneakers';
import { actions } from '../../redux/cart'

const Header:FC = () => {
    const dispatch = useDispatch()
    const {totalPrice} = useSelector(({cartReducer}:RootState) => cartReducer)
    const {isAuth} = useSelector(({userReducer}:RootState) => userReducer)

    const onLogout=async()=>{
        dispatch(actionsSneakers.isFetching(true))
        await dispatch(actionsUser.logout())
    }
    ...
    return (
        <div className={s.header}>
            ...
            <div className={s.cart} onClick={()=>dispatch(actions.openCart(true))}>
            ...
            <Link to='/registration'>
                <svg className={s.user} width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
